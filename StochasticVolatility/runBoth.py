...
def main(group, fig_no, savetrace):
    ...
    data_numAll = pd.read_csv(group + '.csv', header = None)
    ...
    with pm.Model() as model_old:
        ...
        #sigma   = pm.Uniform('sigma', 0.0001,0.5)
    
        sigmab  = pm.Exponential('sigmab', 1, testval=.1) 
        #sigmab  = pm.Uniform('sigmab', 0.0001,0.5)
    ...
    with model_old:
        start = pm.find_MAP(vars=[x, sigma,sigmab,beta_0], fmin=optimize.fmin_l_bfgs_b)


    with model_old:
        step1  = pm.NUTS(vars=[x, sigma, sigmab, beta_0],scaling=start, gamma=.25)
        start2 = pm.sample(500, step1, start=start)[-1]

    # Start next run at the last sampled position.
        step2  = pm.NUTS(vars=[x, sigma, sigmab, beta_0],scaling=start2, gamma=.55)
        trace1 = pm.sample(1000, step2, start=start2,  progressbar=True)
    

    plt.figure(50)
    fig = pm.traceplot(trace1)  
...
if __name__ == "__main__":
    ...
    plt.close('all')
    savetrace = {}
    p_sevo   = main('sevo', 1, savetrace)
    p_oxyg   = main('oxyg', 2, savetrace)
