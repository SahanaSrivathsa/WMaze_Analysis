...
def main(group,anType,sessionNum):
    ...
    with pm.Model() as model_add:
        ...
        x = np.arange(data_numAll.shape[1])
        # GaussianRandomWalk('x', sd=sigma, init=pm.Normal.dist(mu=0.0, sd=0.01), shape=data_numAll.shape[1])
        pm.Deterministic('p', tinvlogit(beta_1 * x + betaPop0))

        for rat in range(numAnimals):
            stp = 'p{0}'.format(rat)
            stn = 'n{0}'.format(rat)
            pn = pm.Deterministic(stp, tinvlogit(beta_1 * x + beta_0[rat]))
            pm.Binomial(stn, p=pn, n=np.asarray(data_denom[rat:(rat + 1)]),
                        observed=np.asarray(data_numAll[rat:(rat + 1)]))


    with model_add:
        step1 = pm.NUTS(vars=[beta_1, sigmab, beta_0], gamma=.25)
        start2 = pm.sample(2000, step1)[-1]

        # Start next run at the last sampled position.
        step2 = pm.NUTS(vars=[beta_1, sigmab, beta_0], scaling=start2, gamma=.55)
        trace1 = pm.sample(5000, step2, start=start2, progressbar=True)

    print('---------')
    (waic_val, waic_se, waic_p) = pm.stats.waic(model=model_add, trace=trace1, n_eff=True)
    dic_val = pm.stats.dic(model=model_add, trace=trace1)
    ...
    dataDir = '/Users/adelekap/Documents/WMaze_Analysis/Paper/data/GLM/'+sessionNum+'Sessions/'
    txtFile = '{0}{1}_{2}_learningTrials.txt'.format(dataDir,dtype,group)
    ...
    with open('{0}{1}{2}DATASET.txt'.format(dataDir,group,anType), 'w') as data:
...
if __name__ == "__main__":
    ...
    with open('{0}{1}certainty.txt'.format(dataDir,anType),'w') as c:
