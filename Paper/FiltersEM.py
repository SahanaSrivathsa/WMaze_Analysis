...
def NewtonSolve(x_prior, sigma_prior, N, Nmax, mu):
	...
	for i in range(30): 
		...
		if np.abs(x-it)<1e-10:
			#print 'cvged at second attempt in ',i
			return x
		it = x
	...
	for i in range(30): 
		...
		if np.abs(x-it)<1e-10:
			#print 'cvged at third attempt in ',i
			return x
		it = x
"""
------------------------------------------------------------------------------
...
--------------------------------------------------------------------
"""
def EM(xx, Nmax, mu, sigma2e, x_init, sigma_init):
	...
	while diff_its>0.00001 and its <= max_its:
		...
		x_prior,x_post,sigma2_prior,sigma2_post, ape = FwdFilterEM(xx.values, Nmax.values, 1, x_init, sigma_init, sigma2e, mu) 
...
------------------------------------------------------------------------------
"""
def FwdFilterEM(y,Nmax,delta,x_init,sigma2_init,sigma2e, mu):
	...
	for t in range(1,T+1):
		...
		x_post[t]  = NewtonSolve(x_prior[t],sigma2_prior[t],y[t-1],Nmax[t-1],mu)
